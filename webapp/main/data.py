...
def get_task_db(session):
    task = session[TASK]
    coll_name = 'task_{}'.format(task)
    return db_[coll_name]


def get_crowd_info_db(session):
    if session[DEBUG]:
        return db_.crowd_info_test
    return db_.crowd_info


def get_crowd_data_db(session):
    coll_name = 'crowd_data_{}'.format(session[TASK])
    if session[DEBUG]:
        coll_name += '_test'
    return db_[coll_name]
...
def load_label_tasks(session):
    ...
    coll_task = get_task_db(session)
    coll_crowd_data = get_crowd_data_db(session)
